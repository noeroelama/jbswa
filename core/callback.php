...
if ($array['category'] == 'private' && $mysecret == $secret) {
    ...
    $pn     = $array["pushname"];
    $sid    = $array["id"];
    $pesan  = str_replace("`", "", $array["message"]);
    ...
    $keys   = explode("#", $pesan);
    $kuncis = explode(" ", $pesan);
    $keys   = array_map('trim', $keys);
    $kuncis = array_map('trim', $kuncis);
    $key    = strtolower(trim($keys[0]));
    $kunci  = strtolower(trim($kuncis[0]));
    ...
    switch ($kunci) {
        case 'info':
            switch (count($kuncis)) {
                case 1:
                    $balas  = "Untuk mendapatkan informasi administrasi siswa, silakan ketik format berikut:";
                    $balas  .= $dformat; // dari config
                    break;

                case 2:
                    $balas = "Mohon memasukkan PIN dan jenis administrasi yang diperlukan. Berikut format yang tersedia:";
                    $balas .= $dformat;
                    break;

                case 3:
                    $balas = "Mohon memasukkan jenis administrasi yang diperlukan. Berikut format yang tersedia:";
                    $balas .= $dformat;
                    break;

                case 4:
                    sendreact($wa, $sid, "⏳", $base_url, $token);
                    // $balas = "Proses dulu!";
                    $nis    = $kuncis[1];
                    $pin    = $kuncis[2];

                    // $res    = $conn->query("SELECT * FROM jbsakad.siswa WHERE nis='$nis' AND pinsiswa='$pin'");
                    $res    = $conn->query("SELECT s.nis, s.nama, s.tahunmasuk, a.angkatan, a.departemen, k.kelas, s.tmplahir, s.tgllahir, s.kelamin, s.alamatsiswa, t.tingkat, s.pinsiswa, s.namaayah, s.namaibu, s.hportu as hportu1, s.info1 as hportu2, s.info2 as hportu3, k.replid as idkelas FROM jbsakad.siswa s LEFT JOIN jbsakad.angkatan a ON a.replid=s.idangkatan LEFT JOIN jbsakad.kelas k ON k.replid=s.idkelas LEFT JOIN jbsakad.tingkat t ON t.replid=k.idtingkat WHERE k.aktif=1 AND s.aktif=1 AND a.aktif=1 AND t.aktif=1 AND s.nis='$nis' AND s.pinsiswa='$pin' ORDER BY tingkat");

                    if (mysqli_num_rows($res) > 0) {
                        $row        = mysqli_fetch_assoc($res);
                        $nama       = $row['nama'];
                        $dept       = $row['departemen'];
                        switch (strtolower($kuncis[3])) {
                            case 'keu':
                            case 'keuangan':
                                //tahunbuku aktif
                                                ...
                                                if (mysqli_num_rows($res) > 0) {
                                                    ...
                                                    if (mysqli_num_rows($res2) > 0) {
                                                        while ($row3 = mysqli_fetch_assoc($res2)) {
                                                            ...
                                                            $rw .= "▪️▪️ " . $row3['tanggal'] . ' | Jml. ' . rupiah($row3['jumlah']) . "\n";
                                                            $tb += $row3['jumlah'];
                                                            $td += $row3['info1'];
                                                        }
                                                    } else {
                            ...
                            default:
                                $balas = "Layanan dengan kata kunci *" . trim($kuncis[3]) . "* belum tersedia!";
                                break;
                        }
                    } else {
                        $balas = "NIS atau PIN Siswa tidak sesuai dengan data kami. Silakan ulangi dengan data NIS dan PIN siswa aktif yang sesuai, atau hubungi Staf Tata Usaha!";
                    }
    ...
    //     default:
    //         $keys2   = explode(" ", $pesan, 2);
    //         $key2 = $keys2[0];
    //         switch (strtolower(trim($key2))) {
    //             case 'kritik':
    //             case 'saran':
    //             case 'pesan':
    //                 if (count($keys2) == 2) {
    //                     $isipesan = $keys2[1];
    //                     $conn->query("INSERT INTO `jbssms`.`kritiksaran` (`sender`, `senddate`, `message`, `from`, `Status`, `type`) VALUES ('$wa', '$waktu', '$isipesan', '$pn', 1, '$key2')");
    //                     $balas = "Terima kasih, *$key2* telah berhasil disimpan di sistem!";
    //                 }
    //                 break;
